...
//Code Here
  let postTaxPrices = prices.map((element, index, prices) => {
   return element * 1.07
})
...
//Code Here
let totalPopulation = populations.reduce((acc, e) => acc += e, 0);
...
let orderTotals = orders.map(orders => orders.price * orders.tax, + orders.price * orders.tax);
...
let bobsTotal = purchases.reduce((acc, own) => {
    if(own.owner === 'Bob'){
      acc += own.price;
    }
    return acc;
